...
def update_readme():
    ...
    for repo_name, added_date in repos:
        ...
        if repo:
            ...
            # Get stars for the repository
            stars = repo.stargazers_count
            
            repo_info.append((repo, last_commit, time_ago, added_date, stars, twitter_user))
    ...
    with open('README.md', 'w') as f:
        ...
        f.write("| Repository | Stars | Last Commit | Added On | Twitter |\n")
        f.write("|------------|-------|:-----------:|:--------:|---------|\n")

        total_repos = 0
        newest_project = None
        
        for repo, last_commit, time_ago, added_date, stars, twitter_user in repo_info:
            total_repos += 1
            ...
            sleepy_tag = " 😴" if last_commit < datetime.now(last_commit.tzinfo) - timedelta(days=180) else ""
            
            twitter_link = f"[{twitter_user}](https://twitter.com/{twitter_user})" if twitter_user else "N/A"

            f.write(f"| [{repo.full_name}]({repo.html_url}){new_tag}{sleepy_tag} | {stars} | {last_commit.strftime('%Y-%m-%d')} | {added_date} | {twitter_link} |\n")
